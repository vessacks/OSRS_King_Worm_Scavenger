...
#this is a test idea that might make scripting easier. if it does, it should become its own import for future programs. 
class Meta_Action:
    def _init_(self, vision_object, action_object, threshold = .85, name = '\'no name given\'', look_timeout = 5):
...
REPLACE_face_size = [48,93]
REPLACE_click_mask = cv.imread('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\Varrock Firemaker\\image library\\REPLACE_click_mask.png',0)
REPLACE_action = Action('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\Varrock Firemaker\\image library\\REPLACE.png', click_mask= REPLACE_click_mask, face_size= REPLACE_face_size)
REPLACE_meta_action = Meta_Action(vision_object = REPLACE_vision, action_object = REPLACE_action, threshold = REPLACE_THRESHOLD, name = 'REPLACE', timeout = 3)


# in bank
REPLACE_vision_mask= cv.imread('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\king worm\\image library\\REPLACEvision_mask.png', cv.IMREAD_COLOR)
REPLACE_vision= Vision('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\king worm\\image library\\bank.png', method = cv.TM_CCOEFF_NORMED, imread = cv.IMREAD_COLOR, search_mask=REPLACE_vision_mask)
