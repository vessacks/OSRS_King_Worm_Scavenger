...
STOP_1_OUT_THRESHOLD =.5
STOP_2_OUT_THRESHOLD =.5
STOP_3_OUT_THRESHOLD =.45 #lowest observed .49
...
BANK_PREP_THRESHOLD = .3 #lowest observed .35

MAX_WORM_CLICK_TIME = 40 #after this time is elapsed it will stop the worm click grind no matter what
BOAT_OUT_CHECK_THRESHOLD = .8 
BOAT_IN_CHECK_THRESHOLD = .8
...
#boat in check
boat_in_check_vision= Vision('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\king worm\\image library\\boat_in_check.png', method = cv.TM_CCOEFF_NORMED, imread = cv.IMREAD_COLOR)
boat_in_check_meta_action = Meta_Action(vision_object = boat_in_check_vision, threshold = BOAT_IN_CHECK_THRESHOLD, name = 'boat_in_check', look_timeout = 3)
...
if stop_1_outing == 'timeout':
    ...
    exit()
time.sleep(16.5 + abs(np.random.normal(0,1.2)))
#measured 17.51s walking
...
if stop_2_outing == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(13 + abs(np.random.normal(0,.7)))
#measured 13s walking
...
if stop_3_outing == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(11.2 + abs(np.random.normal(0,.4)))
#measured 11.5s walking
...
if stop_4_outing == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(11.2 + abs(np.random.normal(0,.3)))
#measured 11.5s walking
...
if boat_outing == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(9 + abs(np.random.normal(0,.3)))
#measured 9.23s. 

#this will tie everything up until it sees we've got boat options
boat_out_checking = boat_out_check_meta_action.look_color()
if boat_out_checking == 'timeout':
    print('quitting becuase of timeout')
    exit()

#press '1' to choose molch island
key_press('1')
time.sleep(4 + abs(np.random.normal(0,.4)))
#measured 4.09
...
time.sleep(9.89 + abs(np.random.normal(0,.3)))
#measured 9.89s walking
...
time.sleep(1 + abs(np.random.normal(0,.3)))
...
if new_num_worms == old_num_worms:
    print('big trouble: I see %s worms, same as before I clicked the worm source. plowing on anyway...' % new_num_worms)
    ...
    print('weird news: I see %s worms now. Before I saw %s. beginning worm click cycle I guess' %(new_num_worms, old_num_worms))

#worm click cycle
start_time = time.time()
while new_num_worms < 28:
    screenshot = wincap.get_screenshot() 
    inv_worm_allPoints, inv_worm_bestPoint, inv_worm_confidence = inv_worm_vision.find(screenshot, threshold = INV_WORM_THRESHOLD, debug_mode= 'rectangles', return_mode = 'allPoints + bestPoint + confidence')
    if cv.waitKey(1) == ord('q'):
        cv.destroyAllWindows()
    ...
    if time.time() - start_time > MAX_WORM_CLICK_TIME:
        print('worm click timed out after  %ss | I see %s inv_worms | breaking worm click loop but not quitting' %(time.time()- start_time, new_num_worms))
        break

#close inventory 
key_press('f1')
print('pressing f1 to close inventory')
...
if boat_inning == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(7.86 + abs(np.random.normal(0,.3)))
#measured 7.86s walking


#look for boat_in_check
boat_in_checking = boat_in_check_meta_action.look_color()
if boat_in_checking == 'timeout':
    print('quitting becuase of timeout')
    exit()

key_press('3')
print('pressing 3 for shayzien')

#wait a few seconds. same time as out I bet
time.sleep(3.5 + abs(np.random.normal(0,.8)))
...
if stop_4_inning == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(7.34 + abs(np.random.normal(0,.34)))
#measured 7.34
...
if stop_3_inning == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(9.5 + abs(np.random.normal(0,.3)))
#measured 9.5s

#go to stop_2_in
print('travelling stop_3_in to stop_2_in')
stop_2_inning = stop_2_in_meta_action.look_click_color()
if stop_2_inning == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(11.8 + abs(np.random.normal(0,.3)))
#measured 11.8
...
if stop_1_inning == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(13.2 + abs(np.random.normal(0,.3)))
#measured 13.2
...
if bank_prepping == 'timeout':
    print('exitting because of timeout')
    exit()
time.sleep(16 + abs(np.random.normal(0,.2)))
#measured 16
